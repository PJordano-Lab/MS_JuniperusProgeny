...
```{r data loading}
seeds_trees <- read.csv("seeds_trees.csv", sep=";")
```
...
#summary(unique_seeds_trees, csv = "seeds_trees_groups_2_mismatch.csv") # .csv generation for results save
```

This is the end of the computed genetic profile assignment. These results are reviewed and resequenced. Once obtained and named the genetic profiles to which each seed belongs, they will be manually entered into a new dataset together with the seed origin traits (stand-microhabitat-frugivore-deposition site, etc) for further analysis. This dataset is named full_datast.csv in the GitHub repository
...
dataset <- read_delim("full_dataset.csv", 
...
#names(dataset)
dataset <- dataset %>% drop_na(ind) # I remove unlocalised V.vulpes sample :(. I genotyped 25 seeds from a fox sample (labcode = 3041, Stand = SABOJI, Microhabitat = OA, Disperser = V. vulpes, Deposition site = UNKNOWN), and I don't know where I collected it so I have to eliminate it from the analysis. Finally, we will have 1000 seeds available for analysis. 
...
#Patterns of seed rain across AREAS-MH-Frugivores:
...
ggarrange(p1,p2,p3 ,ncol =1,nrow = 3,
          common.legend = FALSE, legend = "bottom",widths = c(1, 1),heights =c(3,3)) # Figure creation
```
...
#  All the deposition sites

```{r}
#Loading locations dataset
data_seedrain <- read_delim("seed_rain_raw.csv", 
...
p8<-ggplot() +
  ...
  theme_light()+ ggtitle("Erithacus rubecola") + ylim(37.0116, 37.0125) + xlim(-6.5338, -6.5320)+ scale_colour_manual(values = col_mh)+ theme(
  axis.text.x = element_blank(),
  axis.text.y = element_blank(),
...
# Lets check the distribution without OA deposition sites
data_seedrain_NOOA<- filter(data_seedrain, mh !="OA")
data_seedrain_NOOA<-as.data.frame(data_seedrain_NOOA)
hist(data_seedrain_NOOA$seed_density_m2) # Poisson
data_seedrain_NOOA$area <- factor(data_seedrain_NOOA$area, levels = c("SABMAR", "SABOJI", "SABCOL"))
data_seedrain_NOOA$mh <- factor(data_seedrain_NOOA$mh, levels = c("PP", "JP", "FF","NF","OA"))
...
dataset_D <- select(dataset_D, -seed)
...
dataset_D_COL <- select(dataset_D_COL, -seed)
#oji
...
dataset_D_OJI <- select(dataset_D_OJI, -seed)
...
dataset_D_MAR <- select(dataset_D_MAR, -seed)
...
dataset_D_JP <- select(dataset_D_JP, -seed)
...
dataset_D_PP <- select(dataset_D_PP, -seed)
...
dataset_D_FF <- select(dataset_D_FF, -seed)
...
dataset_D_NF <- select(dataset_D_NF, -seed)
...
dataset_D_OA <- select(dataset_D_OA, -seed)

#Computing specaccum 

require(vegan)
#sp_JP <- specaccum(dataset_D_JP, "random",permutations=1000)
#sp_PP <- specaccum(dataset_D_PP, "random",permutations=1000)
#sp_NF <- specaccum(dataset_D_NF, "random",permutations=1000)
#sp_FF <- specaccum(dataset_D_FF, "random",permutations=1000)
#sp_OA <- specaccum(dataset_D_OA, "random",permutations=1000)


#plot(sp_JP, ci.type="polygon",col="#555555", lwd=2, ci.lty=0,  xlim = c(0, 300),ylim = c(0, 300),
   #  ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
   #  xlab="Number of dispersed seeds", main="Maternal accumulation in JP")
#plot(sp_PP, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = #c(0, 300),ylim = c(0, 300),
    # ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
   #  xlab="Number of dispersed seeds", main="Maternal accumulation in PP")
#plot(sp_NF, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = #c(0, 300),ylim = c(0, 300),
    # ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
    # xlab="Number of dispersed seeds", main="Maternal accumulation in NF")
#plot(sp_FF, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = #c(0, 300),ylim = c(0, 300),
#     ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
#     xlab="Number of dispersed seeds", main="Maternal accumulation in FF")
#plot(sp_OA, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = #c(0, 300),ylim = c(0, 300),
#     ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
#     xlab="Number of dispersed seeds", main="Maternal accumulation in OA")
```
...
dataset_ERUB <- select(dataset_ERUB, -seed)
dataset_TPHI <- select(dataset_TPHI, -seed)
dataset_TMER <- select(dataset_TMER, -seed)
dataset_VVUL <- select(dataset_VVUL, -seed)
dataset_REM <- select(dataset_REM, -seed)

require(vegan)
#sp_ERUB <- specaccum(dataset_ERUB, "random",permutations=1000)
#sp_TPHI <- specaccum(dataset_TPHI, "random",permutations=1000)
#sp_TMER <- specaccum(dataset_TMER, "random",permutations=1000)
#sp_VVUL <- specaccum(dataset_VVUL, "random",permutations=1000)
#sp_REM  <- specaccum(dataset_REM , "random",permutations=1000)

#plot(sp_ERUB, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = c(0, 100),ylim = c(0, 100), 
#     ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
#     xlab="Number of seeds", main="Maternal accumulation for Erithacus rubecula")
#plot(sp_TPHI, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = c(0, 700),ylim = c(0, 700),  
#     ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
#     xlab="Number of seeds", main="Maternal accumulation for Turdus philomelos")
#plot(sp_TMER, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = c(0, 100),ylim = c(0, 100),  
#     ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
#     xlab="Number of seeds", main="Maternal accumulation for Turdus merula")
#plot(sp_VVUL, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = c(0, 100),ylim = c(0, 100),  
#     ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
#     xlab="Number of seeds", main="Maternal accumulation for Vulpes vulpes")
#plot(sp_REM, ci.type="polygon",col="#555555", lwd=2, ci.lty=0, xlim = c(0, 100),ylim = c(0, 100),  
#     ci.col=rgb(0.33,0.33,0.33,.2), ylab="Mothers richness", 
#     xlab="Number of seeds", main="Maternal accumulation for Remaining species")
```
...
```{r data loading}
matrix <- read_delim("relatedness_matrix.csv", 
